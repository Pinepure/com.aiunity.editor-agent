...
class AiBlenderAgentConfig:
    ...
    def accepted_token_headers(self) -> list[str]:
        return [
            self.PRIMARY_TOKEN_HEADER,
            self.LEGACY_TOKEN_HEADER,
        ]
    ...
    def state_directory_path(self) -> Path:
        return self.root_dir / ".ai_platform_agent" / "blender"
...
class AiToolRegistry:
    ...
    def manifest_hash(self) -> str:
        digest = sha256()
        digest.update(AiBlenderAgentConfig.PROTOCOL_VERSION.encode("utf-8"))
    ...
    def build_manifest_summary(self, config: AiBlenderAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "framework": AiBlenderAgentConfig.FRAMEWORK_NAME,
            "serviceId": AiBlenderAgentConfig.SERVICE_ID,
            "service": AiBlenderAgentConfig.SERVICE_NAME,
            "platformId": AiBlenderAgentConfig.PLATFORM_ID,
            "version": config.service_version,
            "protocolVersion": AiBlenderAgentConfig.PROTOCOL_VERSION,
    ...
    def build_manifest_full(self, config: AiBlenderAgentConfig) -> JsonMap:
        payload = self.build_manifest_summary(config)
        payload["tools"] = [tool.to_full_json() for tool in self.tools]
        return payload

    def build_bundle_index(self, config: AiBlenderAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "platformId": AiBlenderAgentConfig.PLATFORM_ID,
            "manifestHash": self.manifest_hash,
    ...
    def try_build_bundle(self, config: AiBlenderAgentConfig, bundle_id: str) -> JsonMap | None:
        ...
        return {
            ...
            "tools": [
                ...
                for tool in self.tools
                if matches_bundle(tool.id, bundle)
            ],
        }
    ...
    def build_describe_many(self, config: AiBlenderAgentConfig, ids: list[str]) -> JsonMap:
        found: list[JsonMap] = []
        missing: list[str] = []
        for tool_id in ids:
