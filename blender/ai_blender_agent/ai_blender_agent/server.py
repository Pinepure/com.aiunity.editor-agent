...
class AiBlenderAgentServer:
    def __init__(self, config: AiBlenderAgentConfig) -> None:
        self.config = config
    ...
    def _handle_request(self, handler: BaseHTTPRequestHandler, *, method: str) -> None:
        if method == "OPTIONS":
            ...
            handler.send_header(
                ...
                "Content-Type, X-AI-Agent-Token, X-Blender-Ai-Token",
            )
        ...
        try:
            ...
            if not self._is_authorized(handler):
                self._send_json(
                    ...
                    401,
                    {
                        "ok": False,
                        "error": (
                            f"Unauthorized. Provide {AiBlenderAgentConfig.PRIMARY_TOKEN_HEADER} "
                            f"or {AiBlenderAgentConfig.LEGACY_TOKEN_HEADER}."
                        ),
                    },
                )
                return
            ...
            if path_name in {"manifest", "manifest/summary"} and method == "GET":
                ...
                payload = (
                    self.registry.build_manifest_full(self.config)
                    if detail == "full"
                    else self.registry.build_manifest_summary(self.config)
                )
            ...
            if path_name == "tool/describe_many" and method == "POST":
                ...
                payload = self.registry.build_describe_many(self.config, ids)
                self._send_json(handler, 200, payload)
                return
            ...
            if path_name.startswith("result/") and method == "GET":
                ...
                query = parse_qs(parsed.query)
                payload = self.result_handle_store.build_page(
                    handle_id,
                    offset=clamp_number(int((query.get("offset") or ["0"])[0]), 0, 10**9),
                    limit=clamp_number(int((query.get("limit") or ["0"])[0]), 0, 32768),
                )
                if payload is None:
            ...
            if path_name.startswith("call/") and method == "POST":
                ...
                if tool.requires_confirmation and not self.config.full_access_enabled:
                    self._send_json(
                        handler,
                        403,
                        {"ok": False, "toolId": tool_id, "error": "High-risk tool requires --full-access for this adapter."},
                    )
                    return
                body = self._read_body_json(handler)
    ...
    def _is_authorized(self, handler: BaseHTTPRequestHandler) -> bool:
        if not self.config.require_token:
            return True
        provided = handler.headers.get(AiBlenderAgentConfig.PRIMARY_TOKEN_HEADER) or handler.headers.get(
            AiBlenderAgentConfig.LEGACY_TOKEN_HEADER
        )
    ...
    def _read_body_json(self, handler: BaseHTTPRequestHandler) -> dict[str, object]:
        ...
        if not raw.strip():
            return {}
        return ensure_json_object(json.loads(raw))
    ...
    def _build_health_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        return {
            **({"ok": True} if include_ok else {}),
            ...
            "platform": {
                "blenderExecutable": self.config.blender_executable,
                "defaultBlendFile": self.config.default_blend_file or None,
            },
        }
