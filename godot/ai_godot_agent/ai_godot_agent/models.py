...
class AiGodotAgentConfig:
    ...
    SERVICE_ID = "aigodot.agent"
    SERVICE_NAME = "AI Godot Agent"
    PLATFORM_ID = "godot"
    PRIMARY_TOKEN_HEADER = "X-AI-Agent-Token"
    LEGACY_TOKEN_HEADER = "X-Godot-Ai-Token"
    ...
    def state_directory_path(self) -> Path:
        return self.root_dir / ".ai_platform_agent" / "godot"
...
class AiToolRegistry:
    ...
    def manifest_hash(self) -> str:
        digest = sha256()
        digest.update(AiGodotAgentConfig.PROTOCOL_VERSION.encode("utf-8"))
    ...
    def build_manifest_summary(self, config: AiGodotAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "framework": AiGodotAgentConfig.FRAMEWORK_NAME,
            "serviceId": AiGodotAgentConfig.SERVICE_ID,
            "service": AiGodotAgentConfig.SERVICE_NAME,
            "platformId": AiGodotAgentConfig.PLATFORM_ID,
            "version": config.service_version,
            "protocolVersion": AiGodotAgentConfig.PROTOCOL_VERSION,
    ...
    def build_manifest_full(self, config: AiGodotAgentConfig) -> JsonMap:
        payload = self.build_manifest_summary(config)
        payload["tools"] = [tool.to_full_json() for tool in self.tools]
        return payload

    def build_bundle_index(self, config: AiGodotAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "platformId": AiGodotAgentConfig.PLATFORM_ID,
            "manifestHash": self.manifest_hash,
            "bundles": [{"id": bundle.id, "description": bundle.description, "toolCount": len([tool for tool in self.tools if matches_bundle(tool.id, bundle)])} for bundle in self._bundles],
        }

    def try_build_bundle(self, config: AiGodotAgentConfig, bundle_id: str) -> JsonMap | None:
        ...
        return {
            ...
            "platformId": AiGodotAgentConfig.PLATFORM_ID,
            "manifestHash": self.manifest_hash,
            "bundle": {"id": bundle.id, "description": bundle.description},
            "tools": [tool.to_summary_json() for tool in self.tools if matches_bundle(tool.id, bundle)],
        }

    def build_manifest_search(self, config: AiGodotAgentConfig, *, query: str = "", limit: int = 0, namespace_id: str = "", bundle_id: str = "") -> JsonMap:
    ...
    def build_describe_many(self, config: AiGodotAgentConfig, ids: list[str]) -> JsonMap:
        found: list[JsonMap] = []
        missing: list[str] = []
        for tool_id in ids:
