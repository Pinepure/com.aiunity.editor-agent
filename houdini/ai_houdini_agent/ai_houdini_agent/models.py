...
class AiHoudiniAgentConfig:
    root_dir: Path
    host: str = "127.0.0.1"
    port: int = 19779
    require_token: bool = True
    full_access_enabled: bool = False
    hython_executable: str = "hython"
    default_hip_file: str = ""
    tool_timeout_seconds: int = 180
    service_version: str = "0.1.0"

    FRAMEWORK_NAME = "AI Platform Agent Framework"
    PROTOCOL_VERSION = "2.0"
    SERVICE_ID = "aihoudini.agent"
    SERVICE_NAME = "AI Houdini Agent"
    PLATFORM_ID = "houdini"
    PRIMARY_TOKEN_HEADER = "X-AI-Agent-Token"
    LEGACY_TOKEN_HEADER = "X-Houdini-Ai-Token"
    ...
    def state_directory_path(self) -> Path:
        return self.root_dir / ".ai_platform_agent" / "houdini"
...
class AiToolRegistry:
    ...
    def manifest_hash(self) -> str:
        digest = sha256()
        digest.update(AiHoudiniAgentConfig.PROTOCOL_VERSION.encode("utf-8"))
    ...
    def build_manifest_summary(self, config: AiHoudiniAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "framework": AiHoudiniAgentConfig.FRAMEWORK_NAME,
            "serviceId": AiHoudiniAgentConfig.SERVICE_ID,
            "service": AiHoudiniAgentConfig.SERVICE_NAME,
            "platformId": AiHoudiniAgentConfig.PLATFORM_ID,
            "version": config.service_version,
            "protocolVersion": AiHoudiniAgentConfig.PROTOCOL_VERSION,
    ...
    def build_manifest_full(self, config: AiHoudiniAgentConfig) -> JsonMap:
        payload = self.build_manifest_summary(config)
        payload["tools"] = [tool.to_full_json() for tool in self.tools]
        return payload

    def build_bundle_index(self, config: AiHoudiniAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "platformId": AiHoudiniAgentConfig.PLATFORM_ID,
            "manifestHash": self.manifest_hash,
            "bundles": [
                {
                    ...
                    "toolCount": len([tool for tool in self.tools if matches_bundle(tool.id, bundle)]),
                }
                for bundle in self._bundles
            ],
        }

    def try_build_bundle(self, config: AiHoudiniAgentConfig, bundle_id: str) -> JsonMap | None:
    ...
    def build_describe_many(self, config: AiHoudiniAgentConfig, ids: list[str]) -> JsonMap:
        found: list[JsonMap] = []
        missing: list[str] = []
        for tool_id in ids:
