...
from .models import AiFigmaAgentConfig, AiRuntimeState, AiToolExecutionContext, AiToolRegistry, clamp_number, ensure_json_object
from .result_handles import ResultHandleStore
from .tools import default_bundles, register_default_tools


class AiFigmaAgentServer:
    def __init__(self, config: AiFigmaAgentConfig) -> None:
        self.config = config
        ...
        self._tool_context = AiToolExecutionContext(
            ...
            describe_figma_api_token=self._describe_figma_api_token,
            read_figma_api_token=self._read_figma_api_token,
            write_figma_api_token=self._write_figma_api_token,
            clear_figma_api_token=self._clear_figma_api_token,
        )
    ...
    def serve_forever(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_DELETE(self) -> None:  # noqa: N802
                outer._handle_request(self, method="DELETE")
    ...
    def _handle_request(self, handler: BaseHTTPRequestHandler, *, method: str) -> None:
        if method == "OPTIONS":
            ...
            handler.send_header("Access-Control-Allow-Headers", "Content-Type, X-AI-Agent-Token, X-Figma-Ai-Token")
        ...
        try:
            ...
            if not self._is_authorized(handler):
                self._send_json(handler, 401, {"ok": False, "error": f"Unauthorized. Provide {AiFigmaAgentConfig.PRIMARY_TOKEN_HEADER} or {AiFigmaAgentConfig.LEGACY_TOKEN_HEADER}."})
                return
    ...
    def _is_authorized(self, handler: BaseHTTPRequestHandler) -> bool:
        if not self.config.require_token:
            return True
        provided = handler.headers.get(AiFigmaAgentConfig.PRIMARY_TOKEN_HEADER) or handler.headers.get(AiFigmaAgentConfig.LEGACY_TOKEN_HEADER)
    ...
    def _build_health_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        return {
            **({"ok": True} if include_ok else {}),
            "framework": AiFigmaAgentConfig.FRAMEWORK_NAME,
            "service": AiFigmaAgentConfig.SERVICE_NAME,
            "serviceId": AiFigmaAgentConfig.SERVICE_ID,
            "version": self.config.service_version,
            "platformId": AiFigmaAgentConfig.PLATFORM_ID,
            "protocolVersion": AiFigmaAgentConfig.PROTOCOL_VERSION,
            ...
            "platform": {
                "figmaBaseUrl": self.config.figma_base_url,
                "figmaApiToken": self._describe_figma_api_token(),
            },
        }
    ...
    def _build_agent_brief_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        return {
            **({"ok": True} if include_ok else {}),
            "framework": AiFigmaAgentConfig.FRAMEWORK_NAME,
            "platformId": AiFigmaAgentConfig.PLATFORM_ID,
            "summary": "Use Figma token-backed REST tools for files, nodes, comments, variables, and render/image queries while keeping the discovery-first protocol stable.",
            "steps": [
                ...
                "Set or verify the Figma API token before calling file, comment, or variable tools.",
    ...
    def _describe_figma_api_token(self) -> dict[str, object]:
        if self.config.figma_token.strip():
            return {"present": True, "source": "cli", "filePath": str(self.config.figma_token_file_path)}
        env_token = os.environ.get("FIGMA_TOKEN", "").strip()
        if env_token:
            return {"present": True, "source": "env", "filePath": str(self.config.figma_token_file_path)}
        if self.config.figma_token_file_path.exists():
            token = self.config.figma_token_file_path.read_text(encoding="utf-8").strip()
            if token:
                return {"present": True, "source": "file", "filePath": str(self.config.figma_token_file_path)}
        return {"present": False, "source": "none", "filePath": str(self.config.figma_token_file_path)}

    def _read_figma_api_token(self) -> str:
        if self.config.figma_token.strip():
            return self.config.figma_token.strip()
        env_token = os.environ.get("FIGMA_TOKEN", "").strip()
        if env_token:
            return env_token
        if self.config.figma_token_file_path.exists():
            return self.config.figma_token_file_path.read_text(encoding="utf-8").strip()
        return ""

    def _write_figma_api_token(self, token: str) -> None:
        value = token.strip()
        if not value:
            raise RuntimeError("Figma API token must not be empty.")
        self.config.figma_token_file_path.parent.mkdir(parents=True, exist_ok=True)
        self.config.figma_token_file_path.write_text(value, encoding="utf-8")
        self.runtime_state.log("info", "Persisted Figma API token to disk.")

    def _clear_figma_api_token(self) -> None:
        if self.config.figma_token_file_path.exists():
            self.config.figma_token_file_path.unlink()
            self.runtime_state.log("info", "Removed persisted Figma API token.")
