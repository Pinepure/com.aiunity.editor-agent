...
class AiFigmaAgentConfig:
    ...
    SERVICE_ID = "aifigma.agent"
    SERVICE_NAME = "AI Figma Agent"
    PLATFORM_ID = "figma"
    PRIMARY_TOKEN_HEADER = "X-AI-Agent-Token"
    LEGACY_TOKEN_HEADER = "X-Figma-Ai-Token"
    ...
    def state_directory_path(self) -> Path:
        return self.root_dir / ".ai_platform_agent" / "figma"
    ...
    @property
    def figma_token_file_path(self) -> Path:
        return self.state_directory_path / "figma_api_token.txt"
...
class AiToolExecutionContext:
    ...
    describe_figma_api_token: Callable[[], JsonMap]
    read_figma_api_token: Callable[[], str]
    write_figma_api_token: Callable[[str], None]
    clear_figma_api_token: Callable[[], None]
...
class AiToolRegistry:
    ...
    def manifest_hash(self) -> str:
        digest = sha256()
        digest.update(AiFigmaAgentConfig.PROTOCOL_VERSION.encode("utf-8"))
    ...
    def build_manifest_summary(self, config: AiFigmaAgentConfig) -> JsonMap:
        return {"ok": True, "framework": AiFigmaAgentConfig.FRAMEWORK_NAME, "serviceId": AiFigmaAgentConfig.SERVICE_ID, "service": AiFigmaAgentConfig.SERVICE_NAME, "platformId": AiFigmaAgentConfig.PLATFORM_ID, "version": config.service_version, "protocolVersion": AiFigmaAgentConfig.PROTOCOL_VERSION, "manifestHash": self.manifest_hash, "toolCount": self.count, "namespaces": self.namespace_infos, "tools": [tool.to_summary_json() for tool in self.tools]}

    def build_manifest_full(self, config: AiFigmaAgentConfig) -> JsonMap:
        payload = self.build_manifest_summary(config)
        payload["tools"] = [tool.to_full_json() for tool in self.tools]
        return payload

    def build_bundle_index(self, config: AiFigmaAgentConfig) -> JsonMap:
        return {"ok": True, "platformId": AiFigmaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "bundles": [{"id": bundle.id, "description": bundle.description, "toolCount": len([tool for tool in self.tools if matches_bundle(tool.id, bundle)])} for bundle in self._bundles]}

    def try_build_bundle(self, config: AiFigmaAgentConfig, bundle_id: str) -> JsonMap | None:
        bundle = next((item for item in self._bundles if item.id == bundle_id), None)
        if bundle is None:
            return None
        return {"ok": True, "platformId": AiFigmaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "bundle": {"id": bundle.id, "description": bundle.description}, "tools": [tool.to_summary_json() for tool in self.tools if matches_bundle(tool.id, bundle)]}

    def build_manifest_search(self, config: AiFigmaAgentConfig, *, query: str = "", limit: int = 0, namespace_id: str = "", bundle_id: str = "") -> JsonMap:
        ...
        return {"ok": True, "platformId": AiFigmaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "query": query, "namespaceId": namespace_id, "bundleId": bundle_id, "returned": len(tools), "tools": tools}

    def build_describe_many(self, config: AiFigmaAgentConfig, ids: list[str]) -> JsonMap:
        found: list[JsonMap] = []
        missing: list[str] = []
        for tool_id in ids:
        ...
        return {"ok": True, "platformId": AiFigmaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "returned": len(found), "missing": missing, "tools": found}
