...
class AiUnrealAgentServer:
    def __init__(self, config: AiUnrealAgentConfig) -> None:
        self.config = config
    ...
    def _handle_request(self, handler: BaseHTTPRequestHandler, *, method: str) -> None:
        if method == "OPTIONS":
            ...
            handler.send_header("Access-Control-Allow-Headers", "Content-Type, X-AI-Agent-Token, X-Unreal-Ai-Token")
            ...
            if not self._is_authorized(handler):
                self._send_json(
                    ...
                    401,
                    {
                        "ok": False,
                        "error": (
                            f"Unauthorized. Provide {AiUnrealAgentConfig.PRIMARY_TOKEN_HEADER} "
                            f"or {AiUnrealAgentConfig.LEGACY_TOKEN_HEADER}."
                        ),
                    },
                )
                return
    ...
    def _is_authorized(self, handler: BaseHTTPRequestHandler) -> bool:
        if not self.config.require_token:
            return True
        provided = handler.headers.get(AiUnrealAgentConfig.PRIMARY_TOKEN_HEADER) or handler.headers.get(AiUnrealAgentConfig.LEGACY_TOKEN_HEADER)
    ...
    def _build_health_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        return {
            **({"ok": True} if include_ok else {}),
            ...
            "platform": {
                "unrealExecutable": self.config.unreal_executable,
                "projectFile": self.config.project_file or None,
                "remoteControlUrl": self.config.remote_control_url,
            },
        }
