...
class AiUnrealAgentConfig:
    root_dir: Path
    host: str = "127.0.0.1"
    port: int = 19780
    require_token: bool = True
    full_access_enabled: bool = False
    unreal_executable: str = "UnrealEditor"
    project_file: str = ""
    ...
    SERVICE_ID = "aiunreal.agent"
    SERVICE_NAME = "AI Unreal Agent"
    PLATFORM_ID = "unreal"
    PRIMARY_TOKEN_HEADER = "X-AI-Agent-Token"
    LEGACY_TOKEN_HEADER = "X-Unreal-Ai-Token"
    ...
    def state_directory_path(self) -> Path:
        return self.root_dir / ".ai_platform_agent" / "unreal"
...
class AiToolRegistry:
    ...
    def manifest_hash(self) -> str:
        digest = sha256()
        digest.update(AiUnrealAgentConfig.PROTOCOL_VERSION.encode("utf-8"))
    ...
    def build_manifest_summary(self, config: AiUnrealAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "framework": AiUnrealAgentConfig.FRAMEWORK_NAME,
            "serviceId": AiUnrealAgentConfig.SERVICE_ID,
            "service": AiUnrealAgentConfig.SERVICE_NAME,
            "platformId": AiUnrealAgentConfig.PLATFORM_ID,
            "version": config.service_version,
            "protocolVersion": AiUnrealAgentConfig.PROTOCOL_VERSION,
    ...
    def build_manifest_full(self, config: AiUnrealAgentConfig) -> JsonMap:
        payload = self.build_manifest_summary(config)
        payload["tools"] = [tool.to_full_json() for tool in self.tools]
        return payload

    def build_bundle_index(self, config: AiUnrealAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "platformId": AiUnrealAgentConfig.PLATFORM_ID,
            "manifestHash": self.manifest_hash,
    ...
    def try_build_bundle(self, config: AiUnrealAgentConfig, bundle_id: str) -> JsonMap | None:
    ...
    def build_describe_many(self, config: AiUnrealAgentConfig, ids: list[str]) -> JsonMap:
        found: list[JsonMap] = []
        missing: list[str] = []
        for tool_id in ids:
