...
AI 不应预设“Unity 一定有什么工具”，而应先读取 manifest，确认当前有哪些能力、每个工具的参数结构是什么、返回值是什么、风险等级是什么。这样，AI 的能力边界来自运行中的 Unity 编辑器本身，而不是硬编码假设。
...
- 服务与协议信息：健康检查、manifest 获取、Agent 操作手册读取、服务配置查看、最近日志和最近调用记录。
- 编译与诊断：读取编译状态、控制台错误和警告、清理 Console、辅助定位执行失败原因。
- 资产能力：检索资源、读取文本资源、转换 GUID 与路径、分析依赖和反向依赖、刷新 AssetDatabase。
...
2. 先读取 `/manifest`，确认当前已注册的工具能力。
3. 根据任务选择现有工具执行，必要时先检查编译状态和错误信息。
4. 如果没有合适工具，生成新的 Editor Tool 脚本并注册。
5. 等待 Unity 编译完成后重新读取 manifest。
6. 调用新工具继续执行，并根据返回结果做验证或下一步处理。

这套流程让 AI 的执行方式从“猜测 Unity 能做什么”，变成“以当前 Unity 实际可用能力为准”。
...
- Agent 操作手册：`GET /agent` 或包内 `AGENT.md`
