...
from .models import AiAndroidAgentConfig, AiRuntimeState, AiToolExecutionContext, AiToolRegistry, clamp_number, ensure_json_object
from .result_handles import ResultHandleStore
from .tools import default_bundles, register_default_tools


class AiAndroidAgentServer:
    def __init__(self, config: AiAndroidAgentConfig) -> None:
        self.config = config
    ...
    def _handle_request(self, handler: BaseHTTPRequestHandler, *, method: str) -> None:
        if method == "OPTIONS":
            ...
            handler.send_header("Access-Control-Allow-Headers", "Content-Type, X-AI-Agent-Token, X-Android-Ai-Token")
        ...
        try:
            ...
            if not self._is_authorized(handler):
                self._send_json(handler, 401, {"ok": False, "error": f"Unauthorized. Provide {AiAndroidAgentConfig.PRIMARY_TOKEN_HEADER} or {AiAndroidAgentConfig.LEGACY_TOKEN_HEADER}."})
                return
    ...
    def _is_authorized(self, handler: BaseHTTPRequestHandler) -> bool:
        if not self.config.require_token:
            return True
        provided = handler.headers.get(AiAndroidAgentConfig.PRIMARY_TOKEN_HEADER) or handler.headers.get(AiAndroidAgentConfig.LEGACY_TOKEN_HEADER)
    ...
    def _build_health_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        return {"ok": True, "framework": AiAndroidAgentConfig.FRAMEWORK_NAME, "service": AiAndroidAgentConfig.SERVICE_NAME, "serviceId": AiAndroidAgentConfig.SERVICE_ID, "version": self.config.service_version, "platformId": AiAndroidAgentConfig.PLATFORM_ID, "protocolVersion": AiAndroidAgentConfig.PROTOCOL_VERSION, "serverRunning": self.is_running, "requiresToken": self.config.require_token, "acceptedTokenHeaders": self.config.accepted_token_headers, "serverUrl": self.config.server_url, "manifestHash": self.registry.manifest_hash, "toolCount": self.registry.count, "namespaces": self.registry.namespace_infos, "supportsManifestSearch": True, "supportsDescribeMany": True, "supportsResultHandles": True, "supportsBundles": True, "supportsTextChunking": True, "supportsDynamicToolRegistration": False, "recommendedFlow": ["GET /health and compare manifestHash before refreshing capabilities.", "Use POST /manifest/search or GET /manifest/bundle/{id} to narrow candidate tools.", "Use POST /tool/describe_many for exact argument and return schemas before calling tools.", "Use GET /result/{handleId} for additional pages or text chunks when a tool returns a resultHandle.", "Use GET /manifest/full only as a fallback when search is insufficient."], "paths": {"health": "/health", "manifestSummary": "/manifest", "manifestFull": "/manifest/full", "manifestSearch": "/manifest/search", "manifestBundles": "/manifest/bundles", "toolDescribeMany": "/tool/describe_many", "call": "/call/{toolId}", "agent": "/agent", "agentBrief": "/agent/brief", "resultPage": "/result/{handleId}"}, "platform": {"adbExecutable": self.config.adb_executable, "gradleExecutable": self.config.gradle_executable, "emulatorExecutable": self.config.emulator_executable, "projectDir": self.config.project_dir or None, "androidSdkRoot": self.config.android_sdk_root or None}} if include_ok else {"framework": AiAndroidAgentConfig.FRAMEWORK_NAME, "service": AiAndroidAgentConfig.SERVICE_NAME, "serviceId": AiAndroidAgentConfig.SERVICE_ID, "version": self.config.service_version, "platformId": AiAndroidAgentConfig.PLATFORM_ID, "protocolVersion": AiAndroidAgentConfig.PROTOCOL_VERSION, "serverRunning": self.is_running, "requiresToken": self.config.require_token, "acceptedTokenHeaders": self.config.accepted_token_headers, "serverUrl": self.config.server_url, "manifestHash": self.registry.manifest_hash, "toolCount": self.registry.count, "namespaces": self.registry.namespace_infos, "supportsManifestSearch": True, "supportsDescribeMany": True, "supportsResultHandles": True, "supportsBundles": True, "supportsTextChunking": True, "supportsDynamicToolRegistration": False, "recommendedFlow": ["GET /health and compare manifestHash before refreshing capabilities.", "Use POST /manifest/search or GET /manifest/bundle/{id} to narrow candidate tools.", "Use POST /tool/describe_many for exact argument and return schemas before calling tools.", "Use GET /result/{handleId} for additional pages or text chunks when a tool returns a resultHandle.", "Use GET /manifest/full only as a fallback when search is insufficient."], "paths": {"health": "/health", "manifestSummary": "/manifest", "manifestFull": "/manifest/full", "manifestSearch": "/manifest/search", "manifestBundles": "/manifest/bundles", "toolDescribeMany": "/tool/describe_many", "call": "/call/{toolId}", "agent": "/agent", "agentBrief": "/agent/brief", "resultPage": "/result/{handleId}"}, "platform": {"adbExecutable": self.config.adb_executable, "gradleExecutable": self.config.gradle_executable, "emulatorExecutable": self.config.emulator_executable, "projectDir": self.config.project_dir or None, "androidSdkRoot": self.config.android_sdk_root or None}}

    def _build_agent_brief_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        payload = {"framework": AiAndroidAgentConfig.FRAMEWORK_NAME, "platformId": AiAndroidAgentConfig.PLATFORM_ID, "summary": "Use Android SDK, ADB, Gradle, and emulator tools through the discovery-first protocol instead of guessing project or device state.", "steps": ["Call GET /health and reuse cached capabilities while manifestHash stays unchanged.", "Search tools with POST /manifest/search or load a focused bundle with GET /manifest/bundle/{id}.", "Prefer installation, project, and device inspection tools before mutation or build commands.", "Request exact tool schemas with POST /tool/describe_many before calling unfamiliar tools.", "When a tool returns resultHandle, page additional data through GET /result/{handleId} instead of re-running the tool with larger limits."], "paths": {"health": "/health", "manifestSummary": "/manifest", "manifestFull": "/manifest/full", "manifestSearch": "/manifest/search", "manifestBundles": "/manifest/bundles", "toolDescribeMany": "/tool/describe_many", "call": "/call/{toolId}", "agent": "/agent", "agentBrief": "/agent/brief", "resultPage": "/result/{handleId}"}}
        if include_ok:
            payload["ok"] = True
        return payload
