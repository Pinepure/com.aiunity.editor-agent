...
from .models import AiIosAgentConfig, AiRuntimeState, AiToolExecutionContext, AiToolRegistry, clamp_number, ensure_json_object
from .result_handles import ResultHandleStore
from .tools import default_bundles, register_default_tools


class AiIosAgentServer:
    def __init__(self, config: AiIosAgentConfig) -> None:
        self.config = config
    ...
    def _handle_request(self, handler: BaseHTTPRequestHandler, *, method: str) -> None:
        if method == "OPTIONS":
            ...
            handler.send_header("Access-Control-Allow-Headers", "Content-Type, X-AI-Agent-Token, X-Ios-Ai-Token")
        ...
        try:
            ...
            if not self._is_authorized(handler):
                self._send_json(handler, 401, {"ok": False, "error": f"Unauthorized. Provide {AiIosAgentConfig.PRIMARY_TOKEN_HEADER} or {AiIosAgentConfig.LEGACY_TOKEN_HEADER}."})
                return
    ...
    def _is_authorized(self, handler: BaseHTTPRequestHandler) -> bool:
        if not self.config.require_token:
            return True
        provided = handler.headers.get(AiIosAgentConfig.PRIMARY_TOKEN_HEADER) or handler.headers.get(AiIosAgentConfig.LEGACY_TOKEN_HEADER)
    ...
    def _build_health_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        payload = {"framework": AiIosAgentConfig.FRAMEWORK_NAME, "service": AiIosAgentConfig.SERVICE_NAME, "serviceId": AiIosAgentConfig.SERVICE_ID, "version": self.config.service_version, "platformId": AiIosAgentConfig.PLATFORM_ID, "protocolVersion": AiIosAgentConfig.PROTOCOL_VERSION, "serverRunning": self.is_running, "requiresToken": self.config.require_token, "acceptedTokenHeaders": self.config.accepted_token_headers, "serverUrl": self.config.server_url, "manifestHash": self.registry.manifest_hash, "toolCount": self.registry.count, "namespaces": self.registry.namespace_infos, "supportsManifestSearch": True, "supportsDescribeMany": True, "supportsResultHandles": True, "supportsBundles": True, "supportsTextChunking": True, "supportsDynamicToolRegistration": False, "recommendedFlow": ["GET /health and compare manifestHash before refreshing capabilities.", "Use POST /manifest/search or GET /manifest/bundle/{id} to narrow candidate tools.", "Use POST /tool/describe_many for exact argument and return schemas before calling tools.", "Use GET /result/{handleId} for additional pages or text chunks when a tool returns a resultHandle.", "Use GET /manifest/full only as a fallback when search is insufficient."], "paths": {"health": "/health", "manifestSummary": "/manifest", "manifestFull": "/manifest/full", "manifestSearch": "/manifest/search", "manifestBundles": "/manifest/bundles", "toolDescribeMany": "/tool/describe_many", "call": "/call/{toolId}", "agent": "/agent", "agentBrief": "/agent/brief", "resultPage": "/result/{handleId}"}, "platform": {"xcodebuildExecutable": self.config.xcodebuild_executable, "xcrunExecutable": self.config.xcrun_executable, "projectPath": self.config.project_path or None, "workspacePath": self.config.workspace_path or None, "scheme": self.config.scheme or None, "destination": self.config.destination}}
        if include_ok:
            payload["ok"] = True
        return payload

    def _build_agent_brief_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        payload = {"framework": AiIosAgentConfig.FRAMEWORK_NAME, "platformId": AiIosAgentConfig.PLATFORM_ID, "summary": "Use xcodebuild and simctl through the shared discovery-first protocol instead of guessing iOS project, scheme, or simulator state.", "steps": ["Call GET /health and reuse cached capabilities while manifestHash stays unchanged.", "Search tools with POST /manifest/search or load a focused bundle with GET /manifest/bundle/{id}.", "Inspect project, scheme, and simulator state before build or simulator mutation.", "Request exact tool schemas with POST /tool/describe_many before calling unfamiliar tools.", "When a tool returns resultHandle, page additional data through GET /result/{handleId} instead of re-running the tool with larger limits."], "paths": {"health": "/health", "manifestSummary": "/manifest", "manifestFull": "/manifest/full", "manifestSearch": "/manifest/search", "manifestBundles": "/manifest/bundles", "toolDescribeMany": "/tool/describe_many", "call": "/call/{toolId}", "agent": "/agent", "agentBrief": "/agent/brief", "resultPage": "/result/{handleId}"}}
        if include_ok:
            payload["ok"] = True
        return payload
