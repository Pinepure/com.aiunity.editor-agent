...
class AiMayaAgentConfig:
    root_dir: Path
    host: str = "127.0.0.1"
    port: int = 19782
    require_token: bool = True
    full_access_enabled: bool = False
    mayapy_executable: str = "mayapy"
    ...
    SERVICE_ID = "aimaya.agent"
    SERVICE_NAME = "AI Maya Agent"
    PLATFORM_ID = "maya"
    PRIMARY_TOKEN_HEADER = "X-AI-Agent-Token"
    LEGACY_TOKEN_HEADER = "X-Maya-Ai-Token"
    ...
    def state_directory_path(self) -> Path:
        return self.root_dir / ".ai_platform_agent" / "maya"
...
class AiToolDefinition:
    ...
    def to_full_json(self) -> JsonMap:
        return {
            ...
            "argsSchemaJson": self.args_schema_json,
            "returnSchemaJson": self.return_schema_json,
            "danger": self.danger,
...
class AiToolRegistry:
    ...
    def manifest_hash(self) -> str:
        digest = sha256()
        digest.update(AiMayaAgentConfig.PROTOCOL_VERSION.encode("utf-8"))
    ...
    def build_manifest_summary(self, config: AiMayaAgentConfig) -> JsonMap:
        return {
            "ok": True,
            "framework": AiMayaAgentConfig.FRAMEWORK_NAME,
            "serviceId": AiMayaAgentConfig.SERVICE_ID,
            "service": AiMayaAgentConfig.SERVICE_NAME,
            "platformId": AiMayaAgentConfig.PLATFORM_ID,
            "version": config.service_version,
            "protocolVersion": AiMayaAgentConfig.PROTOCOL_VERSION,
            "manifestHash": self.manifest_hash,
            "toolCount": self.count,
            "namespaces": self.namespace_infos,
            "tools": [tool.to_summary_json() for tool in self.tools],
        }

    def build_manifest_full(self, config: AiMayaAgentConfig) -> JsonMap:
        payload = self.build_manifest_summary(config)
        payload["tools"] = [tool.to_full_json() for tool in self.tools]
        return payload

    def build_bundle_index(self, config: AiMayaAgentConfig) -> JsonMap:
        return {"ok": True, "platformId": AiMayaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "bundles": [{"id": bundle.id, "description": bundle.description, "toolCount": len([tool for tool in self.tools if matches_bundle(tool.id, bundle)])} for bundle in self._bundles]}

    def try_build_bundle(self, config: AiMayaAgentConfig, bundle_id: str) -> JsonMap | None:
        bundle = next((item for item in self._bundles if item.id == bundle_id), None)
        if bundle is None:
            return None
        return {"ok": True, "platformId": AiMayaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "bundle": {"id": bundle.id, "description": bundle.description}, "tools": [tool.to_summary_json() for tool in self.tools if matches_bundle(tool.id, bundle)]}

    def build_manifest_search(self, config: AiMayaAgentConfig, *, query: str = "", limit: int = 0, namespace_id: str = "", bundle_id: str = "") -> JsonMap:
        ...
        return {"ok": True, "platformId": AiMayaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "query": query, "namespaceId": namespace_id, "bundleId": bundle_id, "returned": len(tools), "tools": tools}

    def build_describe_many(self, config: AiMayaAgentConfig, ids: list[str]) -> JsonMap:
        found: list[JsonMap] = []
        missing: list[str] = []
        for tool_id in ids:
        ...
        return {"ok": True, "platformId": AiMayaAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "returned": len(found), "missing": missing, "tools": found}
