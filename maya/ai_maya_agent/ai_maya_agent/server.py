...
from .models import AiMayaAgentConfig, AiRuntimeState, AiToolExecutionContext, AiToolRegistry, clamp_number, ensure_json_object
from .result_handles import ResultHandleStore
from .tools import default_bundles, register_default_tools


class AiMayaAgentServer:
    def __init__(self, config: AiMayaAgentConfig) -> None:
        self.config = config
    ...
    def _handle_request(self, handler: BaseHTTPRequestHandler, *, method: str) -> None:
        if method == "OPTIONS":
            ...
            handler.send_header("Access-Control-Allow-Headers", "Content-Type, X-AI-Agent-Token, X-Maya-Ai-Token")
        ...
        try:
            ...
            if not self._is_authorized(handler):
                self._send_json(handler, 401, {"ok": False, "error": f"Unauthorized. Provide {AiMayaAgentConfig.PRIMARY_TOKEN_HEADER} or {AiMayaAgentConfig.LEGACY_TOKEN_HEADER}."})
                return
    ...
    def _is_authorized(self, handler: BaseHTTPRequestHandler) -> bool:
        if not self.config.require_token:
            return True
        provided = handler.headers.get(AiMayaAgentConfig.PRIMARY_TOKEN_HEADER) or handler.headers.get(AiMayaAgentConfig.LEGACY_TOKEN_HEADER)
    ...
    def _build_health_payload(self, *, include_ok: bool = False) -> dict[str, object]:
        return {
            **({"ok": True} if include_ok else {}),
            ...
            "platform": {"mayapyExecutable": self.config.mayapy_executable, "defaultSceneFile": self.config.default_scene_file or None},
        }
