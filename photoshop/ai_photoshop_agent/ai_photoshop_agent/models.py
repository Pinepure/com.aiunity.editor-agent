...
class AiPhotoshopAgentConfig:
    ...
    SERVICE_ID = "aiphotoshop.agent"
    SERVICE_NAME = "AI Photoshop Agent"
    PLATFORM_ID = "photoshop"
    PRIMARY_TOKEN_HEADER = "X-AI-Agent-Token"
    LEGACY_TOKEN_HEADER = "X-Photoshop-Ai-Token"

    def __post_init__(self) -> None:
        object.__setattr__(self, "root_dir", Path(self.root_dir).expanduser().resolve())
        object.__setattr__(self, "bridge_dir", Path(self.bridge_dir).expanduser().resolve())
    ...
    def state_directory_path(self) -> Path:
        return self.root_dir / ".ai_platform_agent" / "photoshop"
    ...
    @property
    def generated_tools_directory_path(self) -> Path:
...
class AiToolExecutionContext:
    ...
    bridge_client: Any
    generated_tool_host: Any
    reload_generated_tools: Callable[..., JsonMap]
...
class AiToolRegistry:
    ...
    def manifest_hash(self) -> str:
        digest = sha256()
        digest.update(AiPhotoshopAgentConfig.PROTOCOL_VERSION.encode("utf-8"))
    ...
    def build_manifest_summary(self, config: AiPhotoshopAgentConfig) -> JsonMap:
        return {"ok": True, "framework": AiPhotoshopAgentConfig.FRAMEWORK_NAME, "serviceId": AiPhotoshopAgentConfig.SERVICE_ID, "service": AiPhotoshopAgentConfig.SERVICE_NAME, "platformId": AiPhotoshopAgentConfig.PLATFORM_ID, "version": config.service_version, "protocolVersion": AiPhotoshopAgentConfig.PROTOCOL_VERSION, "manifestHash": self.manifest_hash, "toolCount": self.count, "namespaces": self.namespace_infos, "tools": [tool.to_summary_json() for tool in self.tools]}

    def build_manifest_full(self, config: AiPhotoshopAgentConfig) -> JsonMap:
        payload = self.build_manifest_summary(config)
        payload["tools"] = [tool.to_full_json() for tool in self.tools]
        return payload

    def build_bundle_index(self, config: AiPhotoshopAgentConfig) -> JsonMap:
        return {"ok": True, "platformId": AiPhotoshopAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "bundles": [{"id": bundle.id, "description": bundle.description, "toolCount": len([tool for tool in self.tools if matches_bundle(tool.id, bundle)])} for bundle in self._bundles]}

    def try_build_bundle(self, config: AiPhotoshopAgentConfig, bundle_id: str) -> JsonMap | None:
        bundle = next((item for item in self._bundles if item.id == bundle_id), None)
        if bundle is None:
            return None
        return {"ok": True, "platformId": AiPhotoshopAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "bundle": {"id": bundle.id, "description": bundle.description}, "tools": [tool.to_summary_json() for tool in self.tools if matches_bundle(tool.id, bundle)]}

    def build_manifest_search(self, config: AiPhotoshopAgentConfig, *, query: str = "", limit: int = 0, namespace_id: str = "", bundle_id: str = "") -> JsonMap:
        ...
        return {"ok": True, "platformId": AiPhotoshopAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "query": query, "namespaceId": namespace_id, "bundleId": bundle_id, "returned": len(tools), "tools": tools}

    def build_describe_many(self, config: AiPhotoshopAgentConfig, ids: list[str]) -> JsonMap:
        found: list[JsonMap] = []
        missing: list[str] = []
        for tool_id in ids:
        ...
        return {"ok": True, "platformId": AiPhotoshopAgentConfig.PLATFORM_ID, "manifestHash": self.manifest_hash, "returned": len(found), "missing": missing, "tools": found}
